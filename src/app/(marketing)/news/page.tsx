      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 bg-[#151515] rounded-[32px] border border-white/[0.05] p-8">
          <input 
            type="text" 
            placeholder="Поиск" 
            className="col-span-full mb-6 px-6 py-4 bg-[#151515] border border-white/[0.05] rounded-[32px] text-sm text-white focus:outline-none focus:ring-1 focus:ring-[#E50914]/30 transition-all"
          />

          {metadata.map((item, i) => (
            <div key={i} className="bg-[#151515] border border-white/[0.05] rounded-[32px] overflow-hidden hover:border-[#E50914]/30 transition-all flex flex-col">
              <img src={item.image} alt={`News ${i}`} className="w-full h-48 object-cover" />
              
              <div className="p-6 flex flex-col flex-1">
                <h3 className="text-white font-bold text-xl mb-3">Успейте купить!</h3>
                <p className="text-[#4C4C4C] text-sm line-clamp-3 mb-6 flex-1">
                  Держите руку на пульсе индустрии: ежедневные обновления и эксклюзивные инсайды о главных цифровых активах 24/7.
                </p>
