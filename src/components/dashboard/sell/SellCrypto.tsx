    ...
    <div className="w-full  p-5 md:p-2 shadow-2xl overflow-hidden">
      ...
      <div className="relative mb-6 md:mb-4 group">
        ...
        <input 
          ...
          className="w-full bg-[#0D0D0D] border border-white/5 rounded-[16px] md:rounded-[20px] py-3.5 md:py-4 pl-12 md:pl-14 pr-4 text-white placeholder:text-[#4C4C4C] outline-none focus:border-[#E50914]/40 transition-all text-sm"
        />
      </div>

      <div className="flex flex-col gap-2 overflow-y-auto max-h-[550px] pr-1 md:pr-2 custom-scrollbar">
            ...
            >
              <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
                ...
                <div className="md:hidden text-right">
                  ...
                  <div className="text-[9px] text-[#4C4C4C] uppercase font-bold">курс</div>
                </div>
              </div>
              ...
              <div className="flex md:flex flex-row md:flex-col justify-between w-full md:w-auto border-t border-white/5 pt-3 md:pt-0 md:border-0">
                <div className="flex flex-col">
                    <span className="text-[10px] md:hidden text-[#4C4C4C] font-bold uppercase mb-1">Ваш баланс:</span>
                    <span className="text-white font-medium text-sm md:text-[15px]">{coin.balance} {coin.symbol}</span>
                    <span className={`text-[10px] md:text-[11px] font-bold uppercase ${coin.value === '$0.00' ? 'text-gray-600' : 'text-[#34C759]'}`}>
                    ≈ {coin.value}
                    </span>
                </div>
                <div className="md:hidden flex items-center">
                    <button 
                    onClick={() => router.push(`/dashboard/sell/${coin.symbol.toLowerCase()}`)}
                    className="bg-[#1A1A1A] text-white hover:bg-[#E50914] px-5 py-2 rounded-lg text-xs font-bold transition-all active:scale-95 border border-white/5"
                    >
                    Продать
                    </button>
                </div>
              ...
              <div className="hidden md:flex justify-end">
                <button 
                  onClick={() => router.push(`/dashboard/sell/${coin.symbol.toLowerCase()}`)}
                  className="bg-[#1A1A1A] text-[#34C759] hover:bg-[#E50914] px-8 py-2.5 rounded-xl text-sm font-bold transition-all active:scale-95 border border-white/5"
                >
                  Продать
                </button>
              </div>
